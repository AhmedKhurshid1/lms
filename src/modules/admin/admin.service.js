...
const loginUser = async (req) => {
  ...
  let admin = await Admin.findOne({ where: { emailAddress: emailAddress } });
  if (!admin) throw new Error("UserNotFound");
  ...
  accessToken = await signAndReturnJWT({ ...admin.dataValues });
  ...
  let foundSession = await findDocument("UserSessions", {userId: admin.userId });

  if (foundSession) {

    await updateDocument("UserSessions", userSessionInstance, { userId: admin.userId });
  } else if (!foundSession) {

    userSessionInstance.userId = admin.userId;
...
const createAdmin = async (req) => {
  ...
  let { emailAddress, password, name, phoneNumber, employee_id, role=Users.role.Admin, ...rest } = req.body;
  ...
  /* Check for Exisitng Email */
  let foundUser = await findDocument('admin', { emailAddress, raw: true })
  if (foundUser) {
    if (foundUser.status >= Users.status.PHONE_VERIFIED)
      throw new Error("EmailAlreadyExists");
    await hardDelete('admin', { emailAddress })
  //   let cognitoUser = await CognitoClient.fetchUser({ Username: emailAddress })
  //   if (cognitoUser.success)
  //     await CognitoClient.deleteUser(emailAddress);
  }
  ...
  /* Check for Exisitng PhoneNumber */
  let foundPhoneNumber = await findDocument('admin', { phoneNumber: instanceToCreate.phoneNumber, raw: true })
  if (foundPhoneNumber) {
    if (foundPhoneNumber.status >= Users.status.PHONE_VERIFIED)
      throw new Error("PhoneAlreadyExists");
    await hardDelete('admin', { phoneNumber: instanceToCreate.phoneNumber })
  //   let cognitoUser = await CognitoClient.fetchUser({ Username: emailAddress })
  //   if (cognitoUser.success)
  //     await CognitoClient.deleteUser(emailAddress);
  }
  ...
  let createdUserEmail = null;
  try {
    userDocument = await Admin.create(instanceToCreate, { transaction: t });
    accessToken = signAndReturnJWT({
      ...userDocument.dataValues,
    });
    // await CognitoClient.createUser(emailAddress, password);
...
const createQuestionType = async (req) => {
  let { type } = req.body;
  ...
  let foundUser = await Admin.findOne({ where: instanceToCreate });

  if (foundUser.type != type) throw new Error("TypeAlreadyExists");

  const t = await sequelizeInstance.transaction();

  const ques = await Question.create(instanceToCreate, { transaction: t });

  return {ques};


  // const { id: user_id } = req.headers.User;
  //   const SQL = new SQL_Interaction({ req });
  //   const { question } = req.body;
  //   let payload = { user_id, question };
  //   const instanceCreated = await SQL.create('doc_question', payload);
  //   response(res, { success: true, message: 'Question created successfully', instanceCreated });
