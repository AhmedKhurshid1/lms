...
// const { sendOTP } = require("./admin.service");
...
const addQuestionType = catchAsyncErrors(async (req, res, next) => {
  const questionTypeadd = await createQuestionType(req);
  response(res, questionTypeadd);
});

const addQuestion = catchAsyncErrors(async (req, res, next) => {
  const questionadd = await createQuestion(req);
  response(res, questionadd);
});
